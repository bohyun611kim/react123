 ...
 */
package kr.co.coinis.webserver.holdport.member.mbr001.service.impl;
...
import kr.co.coinis.webserver.holdport.member.mbr001.dao.HoldportRegisterDAO;
 ...
 * @author : Seongcheol
 */
@Service("holdportRegisterService")
public class HoldportRegisterServiceImpl implements HoldportRegisterService {

	@Resource(name="holdportRegisterDAO")
	private HoldportRegisterDAO holdportRegisterDAO;
	...
	public ResultVO doRegist(ReqInsertMemberInfoVO reqInsertMemberInfoVO) throws Exception {
		...
		// 유저 정보 저장
		reqInsertMemberInfoVO = holdportRegisterDAO.insertMemberInfo(reqInsertMemberInfoVO);

		// 유저 정보 저장 성공 시
		if(reqInsertMemberInfoVO.getRtnCd() == 0) {
			String regDt = holdportRegisterDAO.selectJoinDt(reqInsertMemberInfoVO);
			reqInsertMemberInfoVO.setRegDt(regDt);

			// 유저 확장 정보 저장
			reqInsertMemberInfoVO = holdportRegisterDAO.insertMemberInfoExtra(reqInsertMemberInfoVO);

			// 유저 확장 정보 저장 성공 후
			if(reqInsertMemberInfoVO.getRtnCd() == 0 && reqInsertMemberInfoVO.getUserTypeCd() == 2) {
				// 법인 회원 정보 저장
				holdportRegisterDAO.insertMemberCorpInfo(reqInsertMemberInfoVO);
	...
	public ResultVO registerEmailAuth(ReqEmailAuthVO reqEmailAuthVO) {
		...
		authCodeVO = holdportRegisterDAO.procEmailAuth(authCodeVO);

		// 결과가 참일 경우 회원 상태 코드 가입상태로 변경
		if(authCodeVO.getRtnCd() == 0) {
			reqEmailAuthVO = holdportRegisterDAO.updateMemberStatus(reqEmailAuthVO);
	...
	public int selectMemberStatCd(Map<String, Object> param) {
		return holdportRegisterDAO.selectMemberStatCd(param);
	}
}
