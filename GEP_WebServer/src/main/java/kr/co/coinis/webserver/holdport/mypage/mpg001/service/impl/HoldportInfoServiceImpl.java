 ...
 */
package kr.co.coinis.webserver.holdport.mypage.mpg001.service.impl;
 ...
 * @author : Seongcheol
 */
@Service("holdportInfoService")
public class HoldportInfoServiceImpl implements HoldportInfoService {

	@Resource(name="holdportInfoDAO")
	private HoldportInfoDAO holdportInfoDAO;

	@Override
	public UserInfoVO selectUserInfo(ReqUserInfoVO reqUserInfoVO) {
		return holdportInfoDAO.selectUserInfo(reqUserInfoVO);
	}

	@Override
	public RecommendInfoVO selectRecommendInfo(ReqUserInfoVO reqUserInfoVO) {
		return holdportInfoDAO.selectRecommendInfo(reqUserInfoVO);
	}
	...
	public ResultVO changePassword(PwChangeVO pwChangeVO) {
		...
		// 기존 비밀번호 일치 여부확인
		int cnt = holdportInfoDAO.checkPassword(pwChangeVO);

		// 일치할 경우
		if(cnt == 1) {
			// 신규 비밀번호로 업데이트
			pwChangeVO = holdportInfoDAO.procChangePassword(pwChangeVO);
	...
	public ResultVO changeAuthMeansCdToSms(ReqAuthMeansVO reqAuthMeansVO) {
		...
		UserInfoVO userInfoVO = holdportInfoDAO.selectUserInfo(reqUserInfoVO);
		...
		if(userInfoVO.getMobile() != null && !"".equals(userInfoVO.getMobile())) {
			...
			authCodeVO = holdportInfoDAO.procCheckAuthCode(authCodeVO);

			if(authCodeVO.getRtnCd() == 0) {
				reqAuthMeansVO = holdportInfoDAO.procChangeAuthMeansCd(reqAuthMeansVO);
	...
	public ResultVO changeAuthMeansCdToOtp(ReqAuthMeansVO reqAuthMeansVO) {
		...
		String otpKey = holdportInfoDAO.selectOtpKey(param);
	...
	public ResultVO updateMarketingAgree(ReqMarketingAgreeVO reqMarketingAgreeVO) {
		...
		int cnt = holdportInfoDAO.updateMarketingAgree(reqMarketingAgreeVO);
	...
	public boolean checkLastAuthDateTime(Map<String, Object> param) throws NumberFormatException, ParseException {
		...
		String lastAuth = holdportInfoDAO.selectLastAuthDateTime(param);
	...
	public ResultVO updatePhoneNo(KcpAuthVO kcpAuthVO) throws Exception {
		...
		int duplication = holdportInfoDAO.checkMobileDuplication(kcpAuthVO);

		if(duplication == 0) {
			// 모바일 사용 여부, 인증레벨 확인
			UserInfoVO userInfoVO = holdportInfoDAO.selectAuthLevel(kcpAuthVO);

			// 미사용 중일 경우(인증 레벨 : 1)
			if(userInfoVO.getAuthLevel() == 1) {
				// 회원 추가 정보 저장
				kcpAuthVO.setRegDt(userInfoVO.getRegDt());
				kcpAuthVO = holdportInfoDAO.procUpdateMemberInfoExtra(kcpAuthVO);
				...
				if(kcpAuthVO.getRtnCd() == 0) {
					...
					reqAuthLevelVO.setUserName(kcpAuthVO.getUserName());

					int cnt = holdportInfoDAO.updateMemberInfo(reqAuthLevelVO);
					if(cnt != 1) {
					...
					/*if(reqAuthLevelVO.getRtnCd() == 0) {
						...
						reqSmsAuthVO = holdportInfoDAO.procSetSmsAuth(reqSmsAuthVO);
				...
				if(kcpAuthVO.getUserName().equals(userName)
					...
					// 모바일 번호 업데이트
					int cnt = holdportInfoDAO.updatePhoneNo(kcpAuthVO);
	...
	public Map<String, Object> selectAccessLog(ReqAccessLogVO reqAccessLogVO) {
		...
		result.put("size", holdportInfoDAO.selectAccessLogCnt(reqAccessLogVO));
		result.put("list", holdportInfoDAO.selectAccessLog(reqAccessLogVO));
