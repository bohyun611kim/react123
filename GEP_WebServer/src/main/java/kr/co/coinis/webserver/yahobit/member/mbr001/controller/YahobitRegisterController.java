 ...
 */
package kr.co.coinis.webserver.yahobit.member.mbr001.controller;
 ...
 */
@Controller
public class YahobitRegisterController {

	private static final Logger logger = LoggerFactory.getLogger(YahobitRegisterController.class);
	
	@Resource
	private RedisSessionRepository redisSessionRepository;
	
	@Autowired
	private YahobitRegisterService yahobitRegisterService;
	
	@Autowired
	private CommService commService;
	
	@RequestMapping(value = "alibit/register")
	...
	@RequestMapping(value = "/alibit/register/doRegist", method=RequestMethod.POST)
	@ResponseBody
		...
		try {
			if(bindingResult.hasErrors()) {
				...
				if(result.getRtnCd() == 0) {
					result = yahobitRegisterService.doRegist(vo);
	...
	@RequestMapping(value = "/alibit/emailAuth", method=RequestMethod.GET)
	public ModelAndView registerEmailAuth(HttpServletRequest request, HttpSession session
		...
		try {
			...
			result = yahobitRegisterService.registerEmailAuth(vo);
		} catch (Exception e) {
			logger.error(CommonUtils.getPrintStackTrace(e));
		...
		model.setViewName(exchangeId.toLowerCase() + "/member/mbr001/emailAuth");
		model.addObject("result", result);
		
		return model;
	}
	
	@RequestMapping(value = "/alibit/joining", method = RequestMethod.GET)
	public ModelAndView doJoining(HttpServletRequest request, HttpSession session) {
		ModelAndView model = new ModelAndView();

		model.setViewName(CommonUtils.getPageKey(request) + "/member/mbr001/joining");
		return model;
	}
	
	@RequestMapping(value = "/alibit/joining/reSendMail", method = RequestMethod.POST)
	@ResponseBody
	public ResultVO reEmailSend(HttpServletRequest request, HttpSession session
		...
		try {
			if(StringUtils.chekcEmailAddress(address)) {
				...
				int statCd = yahobitRegisterService.selectMemberStatCd(param);
