 ...
 */
package kr.co.coinis.webserver.yahobit.member.mbr002.service.impl;
 ...
 * @author : Seongcheol
 */
@Service("yahobitLoginService")
public class YahobitLoginServiceImpl implements YahobitLoginService {

	private static final Logger logger = LoggerFactory.getLogger(SignInSuccessHandler.class);

	@Resource(name="yahobitLoginDAO")
	private YahobitLoginDAO yahobitLoginDAO;
	...
	public ResultVO selectLoginMatch(ReqLoginVO reqLoginVO) {
		...
		param = yahobitLoginDAO.updateLastLogin(param);
		...
		logger.info("yahobit selectLoginMatch Login exchangeId["+reqLoginVO.getExchangeId()+"],userId["+reqLoginVO.getId()+"]");

		try {
			failCnt = yahobitLoginDAO.selectFailCnt(fail);
		...
		if(failCnt > 4) {
			...
			LoginMatchResultVO vo = yahobitLoginDAO.selectLoginMatch(reqLoginVO);
			...
			default:	// 아이디 혹은 비밀번호 미일치
				yahobitLoginDAO.updateFailCnt(fail);
	...
	public CustomUserDetails selectMemberInfo(ReqLoginVO reqLoginVO) {
		return yahobitLoginDAO.selectMemberInfo(reqLoginVO);
	}

	@Override
	public AuthCodeVO requestSmsCode(AuthCodeVO authCodeVO) {

		AuthInfoVO authInfoVO = yahobitLoginDAO.selectUserAuthInfo(authCodeVO);

		if(authInfoVO.getAuthMeansCd() == 2) {
			authCodeVO.setAuthMeansKeyVal(authInfoVO.getMobile());
			authCodeVO = yahobitLoginDAO.procInsertAuthCode(authCodeVO);
		} else {
	...
	public AuthCodeVO confirmSmsAuthCode(AuthCodeVO authCodeVO) {
		...
		if(authInfoVO.getAuthMeansCd() == 2) {
			authCodeVO.setAuthMeansKeyVal(authInfoVO.getMobile());
			authCodeVO = yahobitLoginDAO.procCheckAuthCode(authCodeVO);
		} else {
			authCodeVO.setRtnCd(-5015);
			authCodeVO.setRtnMsg("SMS 인증을 사용하고 있지 않습니다.");
		}
	...
	public FailCntVO procSetFailCnt(FailCntVO failCntVO) {
		return yahobitLoginDAO.procSetFailCnt(failCntVO);
	}
}
