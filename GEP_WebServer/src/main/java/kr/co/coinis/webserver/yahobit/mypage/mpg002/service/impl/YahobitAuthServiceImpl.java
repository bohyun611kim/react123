 ...
 */
package kr.co.coinis.webserver.yahobit.mypage.mpg002.service.impl; 
...
import kr.co.coinis.webserver.yahobit.mypage.mpg002.dao.YahobitAuthDAO;
...
/**
 * <pre>
 * kr.co.coinis.webserver.yahobit.mypage.mpg002.service.impl 
 *    |_ YahobitAuthServiceImpl.java
 * 
 * </pre>
...
@Service("yahobitAuthService")
public class YahobitAuthServiceImpl implements YahobitAuthService {

	//private static final Logger LOG = LoggerFactory.getLogger(YahobitAuthServiceImpl.class);

	@Resource(name="yahobitWalletDAO")
	private YahobitWalletDAO yahobitWalletDAO;
	
	@Resource(name="yahobitAuthDAO")
	private YahobitAuthDAO yahobitAuthDAO;
	
	@Override
	public Map<String, Object> call_PR_WAS_SET_OTP_AUTH(Map paramMap) throws SQLException {
		return yahobitAuthDAO.call_PR_WAS_SET_OTP_AUTH(paramMap);
	}

	@Override
	public int updateMemberInfoAtOtp(Map paramMap) throws SQLException {
		return yahobitAuthDAO.updateMemberInfoAtOtp(paramMap);
	}

	@Override
	public boolean checkUserPassword(Map paramMap) throws SQLException {
		return yahobitAuthDAO.checkUserPassword(paramMap);
	}
	
	@Override
	public AuthLevel4VO selectAuthLevel4Info(Map<String, Object> param) {
		return yahobitAuthDAO.selectAuthLevel4Info(param);
	}
	...
	public ResultVO uploadIdCard(ReqIdCardVO reqIdCardVO) throws Exception {
		...
		// 회원 정보 조회 
		String userInfo = yahobitAuthDAO.selectUserInfo(reqIdCardVO);
		
		reqIdCardVO.setRegDt(userInfo);
		
		// 인증 레벨 확인
		String level = yahobitAuthDAO.selectAuthLevel(reqIdCardVO);
		...
		case 3:
			// 추가 정보 저장
			int cnt = yahobitAuthDAO.updateLevel4Request(reqIdCardVO);
			...
			if(cnt == 1) {
				...
				param = yahobitAuthDAO.procInsertIdAuthInfo(param);
	...
	public ResultVO uploadCorpIdCard(ReqIdCardVO reqIdCardVO) throws Exception {
		...
		// 인증 레벨 확인
		String level = yahobitAuthDAO.selectAuthLevelCorp(reqIdCardVO);
			...
			if(cnt == 1) {
				...
				if((int) param.get("rtnCd") == 0) {
					...
					if((int) param.get("rtnCd") == 0) {
						...
						param = yahobitAuthDAO.procInsertMemberCorpAuthInfo(param);
	...
	public ResultVO proveResidence(ReqAuthLevel5VO reqAuthLevel5VO) throws Exception {
		...
		// 주소 저장
		int cnt = yahobitAuthDAO.updateExtraAddress(reqAuthLevel5VO);
		
		// 5레벨 인증 정보 저장
		if(cnt == 1) {
			reqAuthLevel5VO = yahobitAuthDAO.procMemberLevel5Mgr(reqAuthLevel5VO);
	...
	public AuthLevel5VO selectAuthLevel5Info(Map<String, Object> param) {
		return yahobitAuthDAO.selectAuthLevel5Info(param);
	}
}
