...
function drawChart(itemCode, chartKind, mktGrpId) {
	let lang = localStorage.getItem("common/LANGUAGE");
	if(candleChart != null) {
		//candleChart.setLanguage(lang);
		//console.log("candleChart---1->", candleChart);
		...
		candleChart = new TradingView.widget({
			...
			client_id: 'www.amczet.com',
			user_id: 'kaouling74@gmail.com',
			timezone: 'Asia/Seoul',
			theme: thema,
			overrides: {
...
function createAreaChart(itemCode, mktGrpId) {
	...
	console.log("createAreaChart+++:", itemCode);
...
function getAreaData(itemCode, mktGrpId) {
	console.log("getAreaData inner +++:", itemCode);
	...
	$.ajax({
		...
		success : function(data, textStatus) {
			...
			if(price != undefined) {
				...
				//console.log('area chart prices#' + price.length);

				for(var i=0; i<price.length; i++) {
					var cal = price[i] - selected.prevClosePrc;

					//console.log('area chart price[' + i + ']: ' + price[i]);
				...
				for(var i=price.length; i<1440; i++) { up.push(null); down.push(null); }
...
function drawAreaChart(up, down) {
	if(areaChart != undefined) {
		...
		areaChart = c3.generate({
		    ...
		    data: { columns: [ up, down ], types: { up: 'area-spline', down: 'area-spline' }, colors: { up: '#f8413d', down: '#0058d0' }, },
