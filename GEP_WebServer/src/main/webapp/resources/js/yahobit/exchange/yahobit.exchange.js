...
$(document).ready(function() {
	$('#notiAgree').on('click', function() {
		...
		if($check.eq(0).is(':checked') === false) {
			lrt.client('암호화폐 거래 위험고지에 동의 하셔야 합니다', '알림');
		} else {
		...
		if($this.data('type') == 'pnd') {
			...
			if($tbody.data('type') === 'buy') {
				if(nu.isN(prc) && prc !== 0) {
					...
					$tbody.find('input:eq(1)').val(nu.cm(qty, selected.qtyCalcDevPnt));
				} else {
					lrt.client('가격을 입력하셔야 합니다.');
				}
			} else {
			...
			if($tbody.data('type') === 'buy') {
				qty = (selected.fromQty / prc) * per;
				$tbody.find('input:eq(0)').val(nu.cm(qty, selected.qtyCalcDevPnt));
			} else {
				qty = selected.toQty * per;
				$tbody.find('input:eq(0)').val(nu.cm(qty, selected.qtyCalcDevPnt));
			}
			...
			$('#autoQty').val(nu.cm(originTo * per, selected.qtyCalcDevPnt));
			$('#autoAmt').text(nu.cm(originFrom * per, selected.prcCalcDevPnt));
			$('#autoOrdAmt').text(nu.cm(originFrom * per, selected.prcCalcDevPnt));
		}
	});
	...
	$('#mktSellPeccent').on('click', function(e) {
		...
		$('#mktSellQty').val(nu.cm(qty, selected.qtyCalcDevPnt));
		$('#mktSellTotOrder').text(nu.cm(totAmt, selected.amtCalcDecPnt));
	});
	...
	$('#mktBuyQty').on('keydown', function(e) {
		...
		let strQty = $(this).val().replace(/,/g, '')
			...
			, calQty = 0, accQty = 0
			, maxQty = parseFloat($('#sellTotal').text().replace(/,/g, ''))
	...
	$('#mktSellQty').on('keydown', function(e) {
		...
		if(length > 0) {
			...
			for(let i=0; i<length;i++) {
				...
				if(tempQty <= 0) {
					// 최대 주문가능 수량 초과
					totAmt = nu.plus(totAmt, nu.multi(mktBuyHoga[i].prc, calQty));
					if(qty !== parseFloat(strQty)) {
						$('#mktSellQty').val(nu.cm(qty, selected.qtyCalcDevPnt) + extraComma);
					}
					break;
				} else {
	...
	function calAutoByQty(flag) {
		...
		if(qty > originTo || prc > originFrom) {
			$('#autoQty').val(nu.cm(originTo, selected.qtyCalcDevPnt));
			$('#autoAmt').text(nu.cm(originFrom, selected.prcCalcDevPnt));
			$('#autoOrdAmt').text(nu.cm(originFrom, selected.prcCalcDevPnt));
		} else {
			$('#autoAmt').text(nu.cm(prc, selected.prcCalcDevPnt));
			$('#autoOrdAmt').text(nu.cm(prc, selected.prcCalcDevPnt));
		}
	}
	...
	function calTotal(t, flag) {
		...
		if($prc != '' && $qty != '' && $prc != 0 && $qty != 0) {
			...
			if(total === 0) {
				...
				selector.text(nu.cm(nu.nvl(total), selected.prcCalcDevPnt));
			}
		} else {
			selector.text(nu.cm(nu.nvl(total), selected.prcCalcDevPnt));
		}
	...
	$('.up').on('click', function() {
		...
		$input.val(nu.cm(value, selected.prcDspDecPnt));
	...
	$('#canSellAll').on('click', function() {
		lrt.confirm('매도 주문을 일괄 취소하시겠습니까?', '일괄매도취소', function() {
			ajaxOption({
				url:'/alibit/exchange/cancelSellAll',
				data:{
	...
	$('#canBuyAll').on('click', function() {
		lrt.confirm('매수 주문을 일괄 취소하시겠습니까?', '일괄매수취소', function() {
			ajaxOption({
				url:'/alibit/exchange/cancelBuyAll',
				data:{
					mktGrpId:selected.mktGrpId,
					mktId:selected.mktId,
					itemCode:selected.itemCode
	...
	$('#nonContractList').on('click', 'button', function() {
		...
		lrt.confirm('주문을 취소하시겠습니까?', '주문 취소', function() {
			ajaxOption({
				url:'/alibit/exchange/doCancel',
				data:{
					itemCode:selected.itemCode,
...
function getItemInfo() {
	ajaxOption({
		url:'/alibit/exchange/getItemCodeInfo',
		data:{itemCode:itemCode, coin:coin},
		success:function(data) {
			let unit = data.info.itemNm.split('/');
			
			var state = {Page:'yahobit', Url:window.location.pathname + '?coin=' + data.info.itemCode};
...
function searchItemCode(data, headerYn) {
	ajaxOption({
		url:'/alibit/exchange/getMktItemCodeInfo',
		data:data,
		success:function(data) {
			let unit = data.info.itemNm.split('/');
...
function setTitle(data, list) {
	$('#coinTitle').text(data.itemDomesticNm);
	...
	for(let i=0; i<list.length; i++) {
		...
		coinList += '<option ' + dataSet + option + '>' + data.itemDomesticNm + '(' + list[i].itemNm + ')</option>';
	}
...
function setItemCodeSise(data) {
	...
	$('#closePrc').text(nu.cm(data.closePrc, data.prcDspDecPnt))
					.closest('td').attr('class', color);
	$('#daebiRate').text(nu.cm(data.prcDevRate, 2) + '%');
	$('#daebiPrc').text(nu.cm(data.prcDevAmt, data.prcDspDecPnt))
					.prev().attr('class', 'rate ' + icon);
	$('#highPrc').text(nu.cm(data.highPrc, data.prcDspDecPnt))
				.attr('class', getColor(data.highPrc, data.prevClosePrc));
				//.attr('class', compareHigh === 0 ? '':compareHigh > 0 ? 'red':'blue');
	$('#lowPrc').text(nu.cm(data.lowPrc, data.prcDspDecPnt))
				.attr('class', getColor(data.lowPrc, data.prevClosePrc));
				//.attr('class', compareLow === 0 ? '':compareLow > 0 ? 'red':'blue');
	$('.tradeVol').text(nu.cm(data.tradeVol, data.volDspDecPnt));
	$('.tradeAmt').text(nu.cm(data.tradeAmt, data.amtDspDecPnt));
	...
	if(data.basicCoinNo === 10) {
		...
		prcUnit.price = nu.cm(data.tradeAmt, data.amtDecPnt);
		prcUnit.unit = selected.fromUnit;
	}
	
	$('#tradeAmt').text(prcUnit.price);
	$('#amtUnit').text(prcUnit.unit);
	
	$('#week52high').text(nu.cm(data.week52High, data.prcDspDecPnt))
					.attr('class', getColor(data.week52High, data.closePrc))
					.next().text(du.getCstmFrmt(data.week52HighDay, 'yyyyMMdd', '(yyyy-MM-dd)'));
	$('#week52low').text(nu.cm(data.week52Low, data.prcDspDecPnt))
	...
	$('#todayHigh').text(nu.cm(data.highPrc, data.prcDspDecPnt))
					.attr('class', getColor(data.highPrc, data.prevClosePrc))
					.next().text(plusMinus + getCalPer(data.highPrc, data.prevClosePrc, true) + '%')
					.attr('class', getColor(data.highPrc, data.prevClosePrc));
	
	plusMinus = data.lowPrc > data.prevClosePrc ? '+':'';
	$('#todayLow').text(nu.cm(data.lowPrc, data.prcDspDecPnt))
					.attr('class', getColor(data.lowPrc, data.prevClosePrc))
					.next().text(plusMinus + getCalPer(data.lowPrc, data.prevClosePrc, true) + '%')
					.attr('class', getColor(data.lowPrc, data.prevClosePrc));
	
	$('.toQty').text(nu.cm(data.toQty, data.qtyDspDecPnt));
	$('.fromQty').text(nu.cm(data.fromQty, data.prcDspDecPnt));
	$('.ordPrc').val(nu.cm(data.closePrc, data.prcDspDecPnt));
	$('.minAmt').text('(최소 주문금액 ' + nu.cm(selected.minOrdAmt,data.amtCalDecPnt) + ' ' + split[1] + ')');
...
function setAuto() {
	...
	if(autoTo > autoFrom) {
		$('#autoFrom').data('origin', selected.fromQty)
			.text(nu.cm(selected.fromQty, selected.prcDspDecPnt));
		...
		$('#autoFrom').data('origin', selected.toQty*selected.closePrc)
			.text(nu.cm(selected.toQty*selected.closePrc, selected.prcDspDecPnt));
		$('#autoTo').data('origin', selected.toQty)
			.text(nu.cm(selected.toQty, selected.qtyDspDecPnt));
	}
}
...
function getCoinInfo() {
	ajaxOption({
		url:'/alibit/exchange/getCoinInfo',
...
function getRealContract() {
	...
	if($body.children('tr').length < REAL_SIZE) {
		ajaxOption({
			url:'/alibit/exchange/getRealContract',
			data:{
				mktGrpId:selected.mktGrpId,
				itemCode:selected.itemCode,
	...
	for(let i=0; i<length; i++) {
		...
		if(ifTransactionId != null){
			if(ifTransactionId.split('_')[0] == 'AT' || ifTransactionId.split('_')[0] == 'AUTO') {
				...
				lDaebiRate = mark + getCalPer(data[i].contractPrc, selected.prevClosePrc, true) + '%';
		...
		.replace(/{{contractAmt}}/g, nu.cm(data[i].contractPrc*data[i].contractQty, selected.amtCalDecPnt))
		.replace(/{{sellBuyColor}}/g, sellBuy)
		.replace(/{{sellBuy}}/g, data[i].contractRecogCd === 1 ? '매수':'매도')
...
function getDaily() {
	...
	if($body.children('tr').length < DAILY_SIZE) {
		ajaxOption({
			url:'/alibit/exchange/getDailySise',
			data:{
				mktGrpId:selected.mktGrpId,
				itemCode:selected.itemCode,
...
function drawDaily(data, flag) {
	...
	for(let i=0; i<length; i++) {
		let color = data[i].daebiRecogCd === 0 ? '':data[i].daebiRecogCd > 0 ? 'red':'blue'
			...
			, daebiRate = (data[i].prcDevAmt / data[i].closePrc) * 100;
...
function getContract() {
	...
	if($body.children('tr').length < CONTRACT_SIZE) {
		ajaxOption({
			url:'/alibit/exchange/getContract',
			data:{
				mktGrpId:selected.mktGrpId,
				itemCode:selected.itemCode,
				contractDt:$body.children('tr:last-child').data('index')
...
function drawContract(data, flag) {
	if(data != undefined) {
		...
		for(let i=0; i<length; i++) {
			...
			.replace(/{{sellBuy}}/g, value.sellBuyRecogCd == 1 ? '매수':'매도')
			...
			.replace(/{{contractAmt}}/g, nu.cm(value.contractPrc*value.contractQty, selected.amtCalDecPnt, true))
	...
	if(data != undefined) {
		...
		for(let i=0; i<length; i++) {
			...
			innerHtml += template
				...
				.replace(/{{sellBuy}}/g, value.sellBuyCd == 1 ? '매수':'매도')
	...
	for(let i=0; i<mkt.length; i++) {
		...
		for(let j=0; j<item.length; j++) {
			...
			tempHtml += itemTemplate.replace(/{{markOn}}/g, mark)
						...
						.replace(/{{index}}/g, j)
						.replace(/{{coinNm}}/g, item[j].itemDomesticNm)
						.replace(/{{toUnit}}/g, unit[0])
						.replace(/{{fromUnit}}/g, unit[1])
						.replace(/{{color}}/g, color)
						.replace(/{{originClosePrc}}/g, item[j].closePrc)
						.replace(/{{closePrc}}/g, nu.cm(item[j].closePrc, item[j].prcDspDecPnt))
						.replace(/{{originDaebiRate}}/g, item[j].prcDevRate)
						.replace(/{{daebiRate}}/g, plusMinus + nu.cm(Math.abs(item[j].prcDevRate), 2) + '%')
						.replace(/{{daebiPrc}}/g, plusMinus + nu.cm(Math.abs(item[j].prcDevAmt), item[j].prcDspDecPnt))
						.replace(/{{originTradeAmt}}/g, item[j].tradeAmt)
						.replace(/{{tradeAmt}}/g, nu.cm(item[j].tradeAmt, item[j].amtDecPnt))
...
function drawFreeMarket(data) {
	...
	for(let i=0; i<mkt.length; i++) {
		...
		mktHtml += '<p><label><input type="radio" name="yep_deposit" checked="true" value="0"/><span>거래대금</span></label></p>';
		mktHtml += '<p><label><input type="radio" name="yep_deposit" value="1"/><span>예치수량</span></label></p>';
		mktHtml += '</div>';*/
	...
	if(data.balance != undefined && data.balance.length > 0) {
		...
		for(let i=0; i<length; i++) {
			...
			if(temp.coinNo === 10) {
				...
				.replace(/{{avgPrc}}/g, '-')
				.replace(/{{bcUnit}}/g, '')
				.replace(/{{profitRate}}/g, mark + ' ' + nu.cm(profitRate, 2) + '%')
				.replace(/{{profitPrc}}/g, mark + ' ' + nu.cm(profitPrc, temp.prcDspDecPnt))
				.replace(/{{color}}/g, color)
				...
				.replace(/{{coinNm}}/g, temp.symbol)
				.replace(/{{qty}}/g, nu.cm(temp.balanceQty, temp.qtyDspDecPnt))
				.replace(/{{bcPrc}}/g, nu.cm(bcPrc, temp.prcDspDecPnt))
				.replace(/{{avgPrc}}/g, nu.cm(temp.avgPrc, temp.prcDspDecPnt))
...
function doPndSell(target) {
	...
	if(checkValue(p)) {
		let message = '주문수량 : ' + nu.cm(p.ordQty, selected.qtyDspDecPnt) + ' ' + selected.toUnit + '<br>';
		message += '주문가격 : ' + nu.cm(p.ordPrice, selected.prcDspDecPnt) + ' ' + selected.fromUnit + '<br>';
		message += '지정가 매도 주문 하시겠습니까?';
		
		ord.sell(message, '매도 주문', function() {
			ajaxOption({
				url:'/alibit/exchange/doPndSell',
				data:p,
				success:function(data) {
					if(data.rtnCd != 0) {lrt.client(wordbook[data.rtnCd], '');}
				}
...
function doPndBuy(target) {
	...
	if(checkValue(p)) {
		let message = '주문수량 : ' + nu.cm(p.ordQty, selected.qtyDspDecPnt) + ' ' + selected.toUnit + '<br>';
		message += '주문가격 : ' + nu.cm(p.ordPrice, selected.prcDspDecPnt) + ' ' + selected.fromUnit + '<br>';
		message += '지정가 매수 주문 하시겠습니까?';
		
		ord.buy(message, '매수 주문', function() {
			ajaxOption({
				url:'/alibit/exchange/doPndBuy',
				data:p,
				success:function(data) {
					if(data.rtnCd != 0) {lrt.client(wordbook[data.rtnCd], '');}
				}
...
function doMktSell(target) {
	...
	if(p.ordPrice == '') {
		lrt.client('매수 호가가 없습니다');
		return false;
	} else if(checkMktValue(p)) {
		let message = '주문수량 : ' + nu.cm(p.ordQty, selected.qtyDspDecPnt) + ' ' + selected.toUnit + '<br>';
		message += '시장가 매도 주문 하시겠습니까?';
			
		ord.sell(message, '매도 주문', function() {
			ajaxOption({
				url:'/alibit/exchange/doMktSell',
				data:p,
				success:function(data) {
					if(data.rtnCd != 0) {lrt.client(wordbook[data.rtnCd], '');}
				}
...
function doMktBuy(target) {
	...
	if(p.ordPrice == '') {
		lrt.client('매도 호가가 없습니다');
		return false;
	} else if(checkMktValue(p)) {
		let message = '주문수량 : ' + nu.cm(p.ordQty, selected.qtyDspDecPnt) + ' ' + selected.toUnit + '<br>';
		message += '시장가 매수 주문 하시겠습니까?';
	
		ord.buy(message, '매수 주문', function() {
			ajaxOption({
				url:'/alibit/exchange/doMktBuy',
				data:p,
				success:function(data) {
					if(data.rtnCd != 0) {lrt.client(wordbook[data.rtnCd], '');}
				}
...
function doAutoMining(target) {
	...
	if(checkValue(p)) {
		let message = '거래수량 : ' + $('#autoQty').val() + ' ' + selected.toUnit + '<br>';
		/*message += '거래대금 : ' + $('#autoAmt').text() + ' ' + selected.fromUnit + '<br>';*/
		message += '자동채굴 거래 하시겠습니까?';
		
		lrt.confirm(message, '자동채굴', function() {
			ajaxOption({
				url:'/alibit/exchange/doAutoMining',
				data:p,
				success:function(data) {
					if(data.rtnCd != 0) {lrt.client(wordbook[data.rtnCd], '');}
					else {
...
function checkValue(data) {
	if(data.ordQty == '') {
		lrt.client('수량을 입력하세요');
		return false;
	} else if(data.ordPrice == '') {
		lrt.client('가격을 입력하세요');
		return false;
	} else if(selected.minOrdAmt > (data.ordQty * data.ordPrice)) {
		lrt.client('최소 주문 금액은 ' + selected.minOrdAmt + selected.itemNm.split('/')[1] + ' 입니다');
		return false;
	} else if(data.ordPrice != undefined && nu.remainder(data.ordPrice, selected.ordPrcUnit) != 0) {
		lrt.client('호가 단위는 ' + selected.ordPrcUnit + selected.itemNm.split('/')[1] + ' 입니다');
		return false;
	}
...
function addContract(data) {
	$('#contractList').prepend(
			...
			.replace(/{{sellBuy}}/g, data.sellBuyRecogCd == 1 ? '매수':'매도')
			...
			.replace(/{{contractAmt}}/g, nu.cm(data.contractPrice*data.contractQty, selected.amtCalDecPnt, true))
...
function addNonContract(data) {
	...
	$('#nonContractList').prepend($('#NON_CONTRACT_TEMPLATE').html()
			...
			.replace(/{{color}}/g, value.sellBuyRecogCd == 1 ? 'red':'blue')
			.replace(/{{sellBuy}}/g, value.sellBuyRecogCd == 1 ? '매수':'매도')
			.replace(/{{ordType}}/g, value.ordTypeCd == 1 ? '지정가':'시장가')
...
function modNonCotnract(data) {
	...
	$('#nonContractList').children(target).children('td')
		...
		.next().text(nu.cm(data.ordQty - data.nonContractQty, selected.qtyDspDecPnt, true));
}
...
function setBalance(data) {
	if(selected.coinNo == data.coinNo) {
		...
		$('.toQty').text(nu.cm(selected.toQty, selected.qtyDspDecPnt));
	} else if(selected.basicCoinNo == data.coinNo) {
		selected.fromQty = data.availQty;
		$('.fromQty').text(nu.cm(selected.fromQty, selected.prcDspDecPnt));
	}
	...
	if($tr.length > 0) {
		...
		if(coinNo == 10) {
			$td.eq(2).data('sort', qty*prc)
				.children('p:first-child').text( nu.cm(qty, qtyPnt) )
		} else {
			$td.eq(2).data('sort', qty*prc)
				.children('p:first-child').text( nu.cm(qty, qtyPnt) )
				.next().children('span:eq(0)').text( nu.cm(qty*prc, prcPnt) );
			$td.eq(3).data('sort', avg)
				.find('span:eq(0)').text( nu.cm(avg, prcPnt) );
		}
		/*$td.eq(3).find('span:eq(0)').text( nu.cm(qty*avg, prcPnt) )
		$td.eq(4).data('sort', profitRate)
			.children('p:first-child').attr('class', color).text( mark + ' ' + nu.cm(profitRate, 2) + '%' )
			.next().attr('class', color).text( mark + ' ' + nu.cm(profitPrc, prcPnt) );*/
	}
}
...
function reDrawHeader(data) {
	...
	if(selected.closePrc != parseFloat(data.closePrice)) {
		...
		$('#closePrc').text(nu.cm(selected.closePrc, selected.prcDspDecPnt))
						.closest('td').attr('class', color);
		$('#daebiRate').text(nu.cm(data.priceDevRate, 2) + '%');
		$('#daebiPrc').text(nu.cm(data.priceDevAmt, selected.prcDspDecPnt))
						.prev().attr('class', 'rate ' + icon);
		
		selected.highPrc = parseFloat(data.highPrice);
		compareHigh = selected.highPrc - selected.closePrc;
		
		$('#highPrc').text(nu.cm(selected.highPrc, selected.prcDspDecPnt))
						.attr('class', getColor(selected.highPrc, selected.prevClosePrc))
						//.attr('class', compareHigh === 0 ? '':compareHigh > 0 ? 'red':'blue');
		$('#todayHigh').text(nu.cm(selected.highPrc, selected.prcDspDecPnt))
						.attr('class', getColor(selected.highPrc, selected.prevClosePrc))
						.next().text(getCalPer(selected.highPrc, selected.prevClosePrc, false) + '%')
						.attr('class', getColor(selected.highPrc, selected.prevClosePrc));
		
		selected.lowPrc = parseFloat(data.lowPrice);
		compareLow = selected.lowPrc - selected.closePrc;
		
		$('#lowPrc').text(nu.cm(selected.lowPrc, selected.prcDspDecPnt))
						.attr('class', getColor(selected.lowPrc, selected.prevClosePrc))
						//.attr('class', compareLow === 0 ? '':compareLow > 0 ? 'red':'blue');
		$('#todayLow').text(nu.cm(selected.lowPrc, selected.prcDspDecPnt))
						.attr('class', getColor(selected.lowPrc, selected.prevClosePrc))
						.next().text(getCalPer(selected.lowPrc, selected.prevClosePrc, false) + '%')
						.attr('class', getColor(selected.lowPrc, selected.prevClosePrc));
	}
	
	// 거래량, 거래대금
	$('.tradeVol').text(nu.cm(data.tradeVol, selected.volDspDecPnt));
	$('.tradeAmt').text(nu.cm(data.tradeAmt, selected.amtDspDecPnt));

	if(selected.basicCoinNo === 10) {
		prcUnit = getKrwUnit(data.tradeAmt);
	} else {
		prcUnit = {};
		prcUnit.price = nu.cm(selected.tradeAmt, selected.amtDecPnt);
		prcUnit.unit = selected.fromUnit;
	}
	...
	if(selected.week52High != parseFloat(data.weeks52High)) {
		...
		$('#week52high').text(nu.cm(selected.week52High, selected.prcDspDecPnt))
	...
	if(selected.week52Low != parseFloat(data.weeks52Low)) {
		...
		$('#week52low').text(nu.cm(selected.week52Low, selected.prcDspDecPnt))
...
function addRealContract(data) {
	...
	innerHtml = $('#REAL_CONTRACT_LIST_TEMPLATE').html()
		...
		.replace(/{{icon}}/g, icon)
		.replace(/{{color}}/g, color)
		.replace(/{{contractAmt}}/g, nu.cm(parseFloat(data.contractPrice)*parseFloat(data.contractQty), selected.amtCalDecPnt))
		.replace(/{{sellBuyColor}}/g, sellBuy)
		.replace(/{{sellBuy}}/g, contractRecogCd === 1 ? '매수':'매도')
		.replace(/{{toUnit}}/g, selected.toUnit)
		.replace(/{{fromUnit}}/g, selected.fromUnit)
...
function updateDailySise(data) {
	let $tbody = $('#daily')
		...
		if($tr.length > 0) {
			...
			$td.eq(1).text(nu.cm(data.closePrice, selected.prcDspDecPnt))
			.next().attr('class', color).text(mark + ' ' + nu.cm(Math.abs(data.priceDevRate), 2) + '%')
			.next().attr('class', color).children('span:eq(0)').attr('class', 'rate ' + icon)
			.next().text(nu.cm(data.priceDevAmt, selected.prcDspDecPnt));
			$td.eq(4).attr('class', color).text(nu.cm(td4data, selected.volDspDecPnt))
			...
			$tbody.prepend(template
					...
					.replace(/{{daebiRate}}/g, mark + ' ' + nu.cm(Math.abs(data.priceDevRate), 2) + '%')
					.replace(/{{daebiPrc}}/g, nu.cm(data.priceDevAmt, selected.prcDspDecPnt))
					.replace(/{{icon}}/g, icon)
					.replace(/{{color}}/g, color)
					.replace(/{{tradeVol}}/g, nu.cm(contractQty, selected.volDspDecPnt))
					.replace(/{{tradeAmt}}/g, nu.cm(contractPriceAmt, selected.amtDecPnt))
			);
		}
}
...
function reDrawRightMenu(data, flag) {
	...
	if($tr.length > 0) {
		...
		$td.eq(2).data('sort', parseFloat(data.closePrice)).children('p')
					...
					.text(nu.cm(data.closePrice, prcPnt));
		$td.eq(3).data('sort', parseFloat(data.priceDevRate)).children('p:eq(0)')
					.attr('class', upDown).text(mark + ' ' + nu.cm(Math.abs(data.priceDevRate), 2) + '%')
					.next().attr('class', 'daebiPrc ' + upDown).text(mark + ' ' + nu.cm(Math.abs(priceDevAmt), prcPnt));
		...
		if(coinNo === 10) {
			...
			$td.eq(4).data('sort', parseFloat(data.tradeAmt))
				.find('p:first-child').text(nu.cm(data.tradeAmt, data.amtDecPnt))
