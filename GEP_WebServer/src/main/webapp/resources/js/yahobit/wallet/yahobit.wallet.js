...
function getUserBankAccntInfo() {
    $.ajax({
    	url : "/alibit/wallet/wlt001/selectMemberBankAccntInfo"
      , type : 'POST'
      , dataType : 'json'
      , error : function(data,status,error) {
          	//console.log("에러 >> " + JSON.stringify(data));
...
function doProcessInsertUserBankAccntInfo(paramMap) {
    $.ajax({
          url : "/alibit/wallet/wlt001/insertMemberBankAccntInfo"
        , type : 'POST'
        , data : paramMap
        , dataType : 'json'
        , error : function(data,status,error) {
...
function moneyChargingRequest() {
    $.ajax({
        url : "/alibit/wallet/wlt001/requestMoneyDeposit"
        , type : 'POST'
        , data : {
...
function requestMoneyWithdraw(coinNo) {
    // 인증레벨 셋팅
...
function getMoneyWithDrawQty(coinNo) {
    $.ajax({
    	url : "/alibit/wallet/wlt001/checkWithdrawQty"
      , data : { 
    	      coinNo : coinNo
    ...
    $.ajax({
    	url : "/alibit/wallet/wlt001/checkOnceWthrwQty"
      , data : { 
    	      coinNo : coinNo
	   }
      , type : 'POST'
...
function doMoneySmsAuthRequest(paramJson) {
    $.ajax({
        url : "/alibit/wallet/wlt001/requestSmsAuthNumber"
        , data : paramJson
        , type : 'POST'
        , dataType : 'json'
        , error : function(data,status,error) {
...
function checkWithdrawMaxMoney() {
    ...
    var min_withdraw_qty = parseFloat($('#money-tab-2-min-withdraw-qty').val().replace(/,/g,''));
    if(user_input == '') {
    ...
    if(user_input.indexOf('.') > -1) {
        ...
        if(parts[1].length > coin_dcm_pnt) {
            ...
            $('#money-tab-2-withdraw-request-qty').val(user_input);
        }
    }
    ...
    if(user_input_qty > maxQty) {
        $('#money-tab-2-withdraw-request-qty').val( nu.cm(maxQty, coin_dcm_pnt) );
        $('#money-tab-2-real-withdraw-qty').text(nu.cm(realQty, coin_dcm_pnt));
        return false;
        ...
        var withdraw_fee    = parseFloat($('#money-tab-2-withdraw-fee').text());
        $('#money-tab-2-real-withdraw-qty').text(''+ nu.cm((user_input_qty + withdraw_fee), coin_dcm_pnt) );
        return true;
    }
};
...
function doRequestMoneyWithdraw(paramJson) {
    $.ajax({
        url : "/alibit/wallet/wlt001/doRequestWithDraw"
        , data : paramJson
        , type : 'POST'
        , dataType : 'json'
        , error : function(data,status,error) {
...
function getDepositWalletInfo(coinNo) {
    $.ajax({
    	url : "/alibit/wallet/wlt001/selectCoinDepositWalletInfoByUserIdAndCoinNo"
      , data : { 
    	    coinNo : coinNo
	   }
      , type : 'POST'
...
$('#coin-tab-1-user-wallet-address-create').click(function() {
    $.ajax({
    	url : "/alibit/wallet/wlt001/createNewCoinAddress"
      , data : { 
