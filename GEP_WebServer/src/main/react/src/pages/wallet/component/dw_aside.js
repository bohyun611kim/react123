...
const handleError = (e) => {
	e.target.src = "../../img/coin-symbol/BTC.png";
}

const CoinItem1 = ({item, idx, handleClick, active, mine, coinRef}) => {


	//if (mine.length == 0) return null;
	console.log("=======>mine.filter g-__:", item.COIN_SYMBOLIC_NM,">>> no:",item.COIN_NO, ",XXXXXX", coinRef[item.COIN_NO]);

	var symbol = item.COIN_SYMBOLIC_NM;
	var my =  mine.length > 0 && mine.filter(it => symbol == it.POSS_ASSET)


	var balance = my.length > 0  ?  nu.cm(my[0].USABLE_QTY, coinRef[item.COIN_NO]) : 0 ;
	var bybc = my.length > 0  ?  nu.cm(my[0].TOTAL_USABLE_BY_BC,  coinRef[item.COIN_NO]): 0;
...
export default function DWASide() {
			...
			<ul>
				<li>
					<div className="whole-asset-group">
						<span className="txt-box">{getLocaleContent("word","total_balance")}</span>
						<span className="asset-value" id="total_estimated_qty">{user_coin.GRAND_TOTAL_POSS_BY_BC}</span>
						<span className="asset-unit">{getLocaleContent("word","base_unit")}</span>
					</div>
				</li>
				<li>
					<div className="sch-coin-group">
						<p className="sch-coin-input">
							<input type="text" defaultValue="" onChange={handlerCoinFilter} placeholder={getLocaleContent("word","search_coin_desc")} className="sch-input"/>
						</p>
						<span className="checks-item" onClick={e=>setCheck(!ischeck)}>
							<input type="checkbox" checked={ischeck}/>
							<label>{getLocaleContent("word","hold_amount")}</label>
						</span>
